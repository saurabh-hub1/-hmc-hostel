# database.py - Pure MongoDB Version (No PostgreSQL/SQLite)
import os
import json
import hashlib
...
# 🔴 UPDATED: Naya password daal diya
ATLAS_CONNECTION_STRING = "mongodb+srv://hostel_admin:TjLh8XClSHmrXhUB@hosteladmin.jx75bvk.mongodb.net/?appName=hosteladmin"
...
def get_db_connection():
    """Return MongoDB client connection (for compatibility)"""
    return client
...
# ==================== INVENTORY AUTO-DEDUCT ON CHECKIN ====================

def deduct_inventory_for_guests(guest_count):
    """Deduct linen inventory based on number of guests"""
    try:
        items_to_deduct = ['Bed Sheet', 'Pillow Cover', 'Towel', 'Blanket']
        results = []
        
        for item_name in items_to_deduct:
            total_to_deduct = guest_count * 1
            
            # Find the item in inventory
            item = inventory.find_one({'name': item_name})
            if item:
                current_stock = item.get('stock', 0)
                if current_stock >= total_to_deduct:
                    # Deduct stock
                    inventory.update_one(
                        {'name': item_name},
                        {'$inc': {'stock': -total_to_deduct}, '$set': {'last_updated': datetime.now()}}
                    )
                    
                    # Log transaction
                    inventory_logs.insert_one({
                        'item_name': item_name,
                        'action': 'auto_deduct_on_checkin',
                        'quantity': total_to_deduct,
                        'guest_count': guest_count,
                        'timestamp': datetime.now()
                    })
                    
                    results.append({'item': item_name, 'deducted': total_to_deduct, 'success': True})
                else:
                    results.append({'item': item_name, 'deducted': 0, 'success': False, 'reason': f'Insufficient stock! Only {current_stock} left'})
            else:
                results.append({'item': item_name, 'deducted': 0, 'success': False, 'reason': 'Item not found'})
...
# ==================== INVENTORY AUTO-ADD ON CHECKOUT ====================

def add_inventory_on_checkout(guest_count):
    """Add linen inventory back on check-out (based on number of guests)"""
    try:
        items_to_add = ['Bed Sheet', 'Pillow Cover', 'Towel', 'Blanket']
        results = []
        
        for item_name in items_to_add:
            total_to_add = guest_count * 1  # 1 item per guest
            ...
            if item:
                ...
                inventory.update_one(
                    {'name': item_name},
                    {'$inc': {'stock': total_to_add}, '$set': {'last_updated': datetime.now()}}
                )
                
                # Log transaction
                inventory_logs.insert_one({
                    'item_name': item_name,
                    'action': 'auto_add_on_checkout',
                    'quantity': total_to_add,
                    'guest_count': guest_count,
                    'timestamp': datetime.now()
                })
                
                results.append({'item': item_name, 'added': total_to_add, 'success': True})
            else:
                results.append({'item': item_name, 'added': 0, 'success': False, 'reason': 'Item not found'})
        
        return results
        
    except Exception as e:
        print(f"❌ Inventory add on checkout failed: {e}")
        return []

# ==================== WARDEN CHECK-IN / CHECK-OUT ====================

def warden_check_in(app_id, room_number, guest_count=None):
    """Warden check-in with room allocation and automatic inventory deduction"""
    ...
    # Get guest count (if not provided, calculate from guest_details)
    if guest_count is None:
        guest_details = app.get('guest_details', [])
        guest_count = len(guest_details) if guest_details else 1
    
    # 🔴 Deduct inventory based on guest count
    inventory_results = deduct_inventory_for_guests(guest_count)
    ...
    applications.update_one(
        ...
        {'$set': {
            ...
            'actual_guest_count': guest_count
        }}
    )
    ...
    rooms.update_one(
        ...
        {'$set': {
            ...
            'current_guest': app.get('applicant_name')
        }}
    )
    
    # Check if any deduction failed
    failed_items = [r for r in inventory_results if not r.get('success', False)]
    
    if failed_items:
        failed_msg = ", ".join([f"{f['item']}: {f.get('reason', 'Failed')}" for f in failed_items])
        return True, f"⚠️ Check-in successful but inventory alerts: {failed_msg}", inventory_results
    else:
        deducted_msg = ", ".join([f"{r['item']}: {r['deducted']}" for r in inventory_results])
        return True, f"✅ Check-in successful! Inventory deducted: {deducted_msg}", inventory_results

def warden_check_out(app_id):
    """Warden check-out - guest leaves, inventory returns, room becomes vacant"""
    ...
    # Get guest count from application
    guest_details = app.get('guest_details', [])
    guest_count = len(guest_details) if guest_details else 1
    
    allocated_room = app.get('allocated_room')
    
    # 🔴 Add inventory back on checkout (based on guest count)
    inventory_results = add_inventory_on_checkout(guest_count)
    ...
    # Check if any add failed
    failed_items = [r for r in inventory_results if not r.get('success', False)]
    
    if failed_items:
        failed_msg = ", ".join([f"{f['item']}: {f.get('reason', 'Failed')}" for f in failed_items])
        return True, f"⚠️ Check-out successful but inventory alerts: {failed_msg}", inventory_results
    else:
        added_msg = ", ".join([f"{r['item']}: +{r['added']}" for r in inventory_results])
        return True, f"✅ Check-out successful! Inventory returned: {added_msg}", inventory_results
...
# ==================== CHECK-IN / CHECK-OUT FUNCTIONS (Admin) ====================
...
def get_current_occupancy():
    """Get current occupancy for admin with proper app_id"""
    occupied = list(applications.find({
        'room_status': 'Occupied',
        'check_out_date': None
    }).sort('check_in_date', -1))
    ...
    for app in occupied:
        ...
        result.append({
            ...
            'guest_count': len(app.get('guest_details', [])),
            'guest_details': app.get('guest_details', []),
            'room_status': app.get('room_status', 'Occupied'),
            'check_in_date': check_in_str,
...
# ==================== INVENTORY FUNCTIONS (MongoDB Only) ====================
...
def update_inventory_stock(item_name, quantity_change, action, performed_by):
    ...
    try:
        ...
        if new_stock < 0:
            return False, f"Insufficient stock! Current stock: {current_stock}"
